...
\begin{document}
	...
	\node[ultra thick, red, above left] at (pi/4,{sin(pi/4r)}) {\huge $f(x)$};


	\draw[ultra thick, ->] (pi/2,1.1) -- (pi,1.1) node[midway, above, fill=white] {stretch horizontally by $2x$};
	\draw[ultra thick, ->] (-pi/2,-1.1) -- (-pi,-1.1) node[midway, below, fill=white] {stretch horizontally by $2x$};

	\draw[ultra thick,cyan,domain=-1*\c{width}:\c{width},smooth] plot (\x,{sin(.5*\x r)});
	\node[ultra thick, cyan, below right] at (pi/2,{sin(pi/4 r)}) {\huge $f(\frac{1}{2}x)$};
